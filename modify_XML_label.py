...
def stats(opt):
    xml_path = glob.glob(os.path.join(opt.anno_folder_path, '*.xml'))
    object_name_set = {}
    ...
    for p in tqdm(xml_path, desc='[INFO] Rename Label'):
        ...
        for obj in object_set:
            ...
            if name in opt.before_classes:
                obj.find('name').text = opt.after_class
...
if __name__ == '__main__':
    ...
    parser.add_argument('--anno_folder_path', default=None, required=True, help='annotations folder path')
